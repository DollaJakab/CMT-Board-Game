...
export const useStore = create<Store>((set) => ({
    ...
    set(() => {
      return {
        board: new Array(boardRows)
          .fill(null)
          .map(() =>
            new Array(boardCols).fill({ color: "black", value: undefined }),
          ),
        currentLetter: CellTypes.C,
        lines: 0,
        filledCells: 0,
  ...
  setCellValue: (row: number, col: number) => {
    set((state) => {
      ...
      const newValue = Object.values(CellTypes).find(
        (value) => value === newState.currentLetter,
      ) as CellTypes;
      ...
      newState.currentLetter =
        Object.values(CellTypes)[
          (Object.keys(CellTypes).indexOf(newState.currentLetter) + 1) % 3
        ];
